ï»¿import { Roteiro, Dia, Turno, Atracao } from '@/types/roteiro';
...
export default function montarHTML(roteiro: Roteiro): string {
  let conteudo = `
    ...
    <body>
      <h1>Meu Roteiro Ã¢â‚¬â€œ App Orlando</h1>
  `;
  ...
  roteiro.dias.forEach((dia: Dia) => {
    ...
    conteudo += `<h2>${dataFormatada} Ã¢â‚¬â€œ ${dia.tipo}</h2>`;
    ...
    periodos.forEach((periodo) => {
      ...
      if (turno && turno.atividades.length > 0) {
        const horario = turno.inicio && turno.fim ? ` <span class="horario">(${turno.inicio} ÃƒÂ s ${turno.fim})</span>` : '';
        conteudo += `<h3>${periodo[0].toUpperCase() + periodo.slice(1)}${horario}</h3><ul>`;
        
        turno.atividades.forEach((a: Atracao) => {
          conteudo += `<li><strong>${a.nome}</strong> (${a.tipo} Ã¢â‚¬â€œ ${a.area})</li>`;
        });
