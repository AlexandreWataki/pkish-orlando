ï»¿// src/screens/dias/DiaDescanso.tsx
...
import { gerarDiaDescanso } from '@/logic/geradores/gerarDiaDescanso';
...
export const DiaDescanso = ({ diaBruto }: { diaBruto: { id: string; tipo: string } }) => {
  ...
  useEffect(() => {
    async function gerar() {
      ...
      const diaGerado = await gerarDiaDescanso(numero, parkisheiroAtual as any);
      setDia(diaGerado);
    }
    gerar();
  ...
  // ===== Helpers para padronizar REFEIÃƒâ€¡ÃƒÆ’O =====
  ...
  const extrairPreco = (a: any): string | undefined => {
    ...
    if (typeof bruto === 'number') return `$ ${bruto}`; // forÃƒÂ§a espaÃƒÂ§o apÃƒÂ³s $
  ...
  const renderAtividade = (
    ...
    if (atividade.tipo === 'area') {
      const nomeArea = (atividade.area || atividade.titulo || '')
        .replace(/^ÃƒÂ¡rea:\s*/i, '')
        .replace(/^regiÃƒÂ£o:\s*/i, '')
        .trim();

      return (
        <CardSecao
          key={key}
          titulo={`ÃƒÂrea: ${nomeArea}`}
          icone="map-outline"
          tipo="area"
        >
    ...
    if (atividade.tipo === 'refeicao') {
      // tÃƒÂ­tulo com sufixo por perÃƒÂ­odo
      const tituloBase = (atividade.titulo || '').split(' Ã¢â‚¬â€œ ')[0].trim();
      const titulo = sufixoRefeicao ? `${tituloBase} Ã¢â‚¬â€œ ${sufixoRefeicao}` : tituloBase;
      ...
      // textos sÃƒÂ³ com descritivos (sem rÃƒÂ³tulos) e sem "PreÃƒÂ§o mÃƒÂ©dio..." embutido
      ...
      // DescriÃƒÂ§ÃƒÂ£o final: 1Ã‚Âª linha meta + quebra + destaque
      const descricaoComMeta =
        (linhaMeta ? `${linhaMeta}${destaque ? '\n' : ''}` : '') + (destaque || '');

      // >>> SOMENTE o CardRefeicao (sem texto "- CafÃƒÂ© da ManhÃƒÂ£/AlmoÃƒÂ§o/Jantar" depois)
      return (
        <View key={key} style={{ marginBottom: 6 }}>
          <CardRefeicao
            titulo={titulo}
            tipoRefeicao={sufixoRefeicao}
            regiao={undefined}             // nÃƒÂ£o usa 'regiao' como subtÃƒÂ­tulo
  ...
  const renderAtividadesComTransporte = (
    ...
    let baseLat = baseLatAnterior;
    let baseLon = baseLonAnterior;

    atividades.forEach((atividade, index) => {
      const lat = atividade.latitude ?? null;
      const lon = atividade.longitude ?? null;

      if (lat != null && lon != null) {
        if (
          (index === 0 && baseLat != null && baseLon != null) ||
          (index > 0 &&
            atividades[index - 1].latitude !== lat &&
            atividades[index - 1].longitude !== lon)
        ) {
          try {
            const refLat = index === 0 ? baseLat! : atividades[index - 1].latitude!;
            const refLon = index === 0 ? baseLon! : atividades[index - 1].longitude!;
    ...
    return { componentes, ultimaLat: baseLat, ultimaLon: baseLon };
  };
      ...
      {Array.isArray(dia.turnos) &&
        dia.turnos.map((turno: any, i: number) => {
          ...
          const cardRetornoRegiao =
            (turno.periodo === 'noite' || String(turno.titulo).toLowerCase() === 'noite') &&
            ...
            parkisheiroAtual.regiaoHospedagem?.longitude != null
              ? (() => {
                  ...
                  const estimativa = calcularTransporteEstimado(distNum);
                  const destinoNome =
                    parkisheiroAtual.regiaoHospedagem?.nome || 'RegiÃƒÂ£o de Hospedagem';
