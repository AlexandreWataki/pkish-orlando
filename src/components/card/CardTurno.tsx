...
interface Props {
  titulo: string;
  referencia?: string; // <-- Adicionado aqui
  atividades: Atividade[];
  onEscolherAtividade?: (atividade: Atividade) => void;
}

const CardTurno: React.FC<Props> = ({ titulo, referencia, atividades, onEscolherAtividade }) => {
  return (
    <View style={styles.container}>
      <Text style={styles.titulo}>{titulo.toUpperCase()}</Text>

      {referencia && (
        <Text style={styles.referencia}>ðŸ§­ Local de referÃªncia: {referencia}</Text>
      )}
